...
def deserialize(data):
    ...
    keys = ('nodes', 'drug_pair', 'r_label', 'g_label', 'n_labels')
...
def move_to_device_dgl(molecular_graphs, device):
    graphs = []
    num_drugs = len(molecular_graphs)

    for drug_id in range(num_drugs):
        graphs.append(dgl.add_self_loop(molecular_graphs[drug_id]))

    g = dgl.batch(graphs).to(device=device)
...
def collate_dgl(samples):
    ...
    graphs, comb_graphs, drug_pairs, r_labels, g_labels = map(list, zip(*samples))
    
    batched_graph = dgl.batch(graphs)

    batched_comb_graph = dgl.batch(comb_graphs)
    drug_pairs = torch.tensor(drug_pairs, dtype=torch.long)

    return batched_graph, batched_comb_graph, drug_pairs, r_labels, g_labels


def move_batch_to_device_dgl(batch, device):
    g_dgl_pos, comb_g_dgl_pos, drug_pairs_pos, r_labels_pos, targets_pos = batch
    ...
    comb_g_dgl_pos = comb_g_dgl_pos.to(device)

    drug_pairs_pos = drug_pairs_pos.to(device)

    return g_dgl_pos, comb_g_dgl_pos, drug_pairs_pos, r_labels_pos, targets_pos
