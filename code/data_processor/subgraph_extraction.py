...
def generate_subgraph_datasets(params):
    ...
    BKG_file = os.path.join(main_dir, '..', 'data', params.dataset, f"{params.BKG_file_name}.txt")
    print(BKG_file)
    adj_list, triplets, entity2id, relation2id, id2entity, id2relation, rel, num_drug = process_files_ddi(params.file_paths, BKG_file)
...
def links2subgraphs(adj_list, files_data, params, max_label_value=None):
    ...
    print('db_path', params.db_path)
    print('map_size', map_size)
    env = lmdb.open(params.db_path, map_size = round(map_size)+1, max_dbs=6)
...
def get_average_subgraph_size(sample_size, links, adj_list, params):
    ...
    for idx in lst:
        ...
        datum = {'nodes': nodes, 'drug_pair': [n1, n2], 'r_label': r_label, 'g_label': 0, 'n_labels': n_labels, 'subgraph_size': subgraph_size, 'enc_ratio': enc_ratio, 'num_pruned_nodes': num_pruned_nodes}
...
def extract_save_subgraph(args_):
    ...
    datum = {'nodes': pruned_subgraph_nodes, 'drug_pair': [n1, n2], 'r_label': r_label, 'g_label': g_label, 'n_labels': pruned_node_labels, 'subgraph_size': subgraph_size, 'enc_ratio': enc_ratio, 'num_pruned_nodes': num_pruned_nodes}
...
def node_label(subgraph, max_distance=1):
    ...
    dist_to_roots0 = np.clip(ssp.csgraph.dijkstra(sgs_single_root[0], indices=[0], directed=False, unweighted=True, limit=1e6)[:, 1:], 0, max_distance+1)
    dist_to_roots1 = np.clip(ssp.csgraph.dijkstra(sgs_single_root[1], indices=[0], directed=False, unweighted=True, limit=1e6)[:, 1:], 0, max_distance+2)

    dist_to_roots = np.array(list(zip(dist_to_roots0[0], dist_to_roots1[0])), dtype=int)

    target_node_labels = np.array([[0, 1], [1, 0]])
    labels = np.concatenate((target_node_labels, dist_to_roots)) if dist_to_roots.size else target_node_labels

    enclosing_subgraph_nodes = np.where(np.max(labels, axis=1) <= max_distance+2)[0]
...
def subgraph_extraction_labeling(ind, rel, A_list, hop=1, enclosing_subgraph=False, max_nodes_per_hop=None, max_node_label_value=None):
    ...
    labels, enclosing_subgraph_nodes = node_label(incidence_matrix(subgraph), max_distance=2*hop+1)
